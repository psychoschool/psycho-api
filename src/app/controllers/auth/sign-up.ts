...
export const signUp = async (req: Request, res: Response) => {
    const { phone, email, firstName, lastName, password } = req.body as UserRequest
    const hashedPassword = await bcrypt.hash(password, 10)
    const user = await createUser({ phone, email, firstName, lastName, password: hashedPassword })
